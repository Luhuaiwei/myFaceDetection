...
public class FaceDetection {
    ...
    public static Mat prepImage(Mat image) { 
        ...
        Highgui.imwrite("D:\\picture\\gray.jpg", gray);
        //直方图均衡化
        Mat balance = new Mat();
        Imgproc.equalizeHist(gray,balance);
        Highgui.imwrite("D:\\picture\\balance.jpg", balance);
        return balance;
    }
    ...
    public static Mat FeatureOrbLannbased(Mat src, Mat dst){  
        ...
        fd.detect(src, mkp);
        System.out.println("ORB检测特征点用时："+(System.currentTimeMillis() - start)+"ms");
        de.compute(src, mkp, desc); 
        System.out.println("ORB特征点描述子用时："+(System.currentTimeMillis() - start)+"ms");
        ...
        for (int i = 0; i < mats.length; i++) {  
            double dist = mats[i].distance;  
            if (dist < 0.2f) {goodMatch.add(mats[i]);}}  
    ...
    public static Mat FeatureSurfBruteforce(Mat src, Mat dst){  
        ...
        long start = System.currentTimeMillis();
        fd.detect(src, mkp);  
        System.out.println("SURF检测特征点用时："+(System.currentTimeMillis() - start)+"ms");
        de.compute(src, mkp, desc); 
        System.out.println("SURF特征点描述子用时："+(System.currentTimeMillis() - start)+"ms");
        Features2d.drawKeypoints(src, mkp, src); 
 ...
 /*       double maxDist = 800;  
    ...
    public static Mat FeatureSiftLannbased(Mat src, Mat dst){ 
        ...
        long start = System.currentTimeMillis();
        fd.detect(src, mkp);  
        System.out.println("SIFT检测特征点用时："+(System.currentTimeMillis() - start)+"ms");
        de.compute(src, mkp, desc); 
        System.out.println("SIFT特征点描述子用时："+(System.currentTimeMillis() - start)+"ms");
        Features2d.drawKeypoints(src, mkp, src); 
